...
function filelist = match_trial_to_behavior(params_struct)

data_path = params_struct.data_path; 
ml_path = params_struct.ml_path; 

filelist_xlsx = fullfile(ml_path, 'filelist_for_classification.xlsx'); % list of relevant files for each subject
ecog_localprocessed_topdir = fullfile(data_path, 'LocalProcessed'); % directory containing analyzed ecog trial data folders for each subject
target_words_file = fullfile(ml_path, 'Target_Words.mat'); % list of target words to be pronounced
stim_index_key_filename = fullfile(ml_path, 'stim_index_key.xlsx'); % numerical indices for stim features
