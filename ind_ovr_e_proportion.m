...
for class_label_idx = 1:length(class_labels)
    ...
    stim_e_sig_prop = icdf('Binomial', 1-p_value/length(class_ovr_idx_range), total_stim_e, p_value) / total_stim_e;
    ...
    onset_e_sig_prop = icdf('Binomial', 1-p_value/length(class_ovr_idx_range), total_onset_e, p_value) / total_onset_e;
...
%     class_stim_onset_prop_fig = figure('FileName', sprintf('sig_e_ovr_proportions_%s.png', class_label));
%     class_stim_onset_prop_fig.WindowState = 'maximized';
%     class_stim_onset_prop_fig.Units = 'normalized';
%     
%     sgtitle(sprintf('Ratio of electrodes considered significant\nto total electrodes per alignment condition\nClass Label: %s. %s', plaintext(class_label), plainformat(times_folder)),'FontSize', title_size);
% %     title(sprintf('Class Label: %s. %s', plaintext(class_label), plainformat(times_folder)),'FontSize', title_size);
%     
%     subplot(1, 2, 1);
%     ax1 = gca; 
