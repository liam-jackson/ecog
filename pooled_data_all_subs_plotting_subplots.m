function pooled_data_all_subs_plotting_subplots(params_struct)
...
for sub_num_idx = 1:length(sub_nums)
    ...
    sub_fig = figure(sub_num_idx);
    sgtitle(sprintf('Mean Accuracies for LDA Models trained on %s Data', sub_str));
    ...
    sub_fig.Name = sub_str;
    sub_fig.NumberTitle = 'off';
    sub_fig_filename = sprintf('%s_mean_acc_bar_plot.png', sub_str);
    sub_fig_filename_full = fullfile(mRMR_path, times_folder, sub_fig_filename);
    saveas(gcf, sub_fig_filename_full);    
end        
...
all_fig = figure();
sgtitle('Mean Accuracies for LDA Models trained on electrode data from all subjects');
...
all_fig.Name = 'All';
all_fig.NumberTitle = 'off';
all_subs_fig_filename = sprintf('all_subs_mean_acc_bar_plot.png');
all_subs_fig_filename_full = fullfile(mRMR_path, times_folder, all_subs_fig_filename);
saveas(gcf, all_subs_fig_filename_full);
