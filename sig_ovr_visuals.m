...
for class_label_idx = 1:length(class_labels)
    ...
    %%%% class_label Figure
    sig_e_class_label_auc_fig = figure('FileName', sprintf('sig_e_ovr_%s.png', class_label)); 
    ...
    title({'Mean AUCs of Classifiers Corresponding to', 'Statistically Significant Electrodes'}, 'FontSize', title_size);
    subtitle(sprintf('%s, One-versus-Rest, Averaged across subjects. %s', plaintext(class_label), plainformat(times_folder)), 'FontSize', subtitle_size);
    hold off;
